...
if ENV=staging:
    it loads the staging settings file

if ENV=prod:
    it loads the production settings file called prod
    if it fails to load the prod settings file, it reverts to staging
"""
...
if env != "local":
    if env in ("prod", "production"):
        try:
            ...
            print(":| No production settings found, reverting to staging")
            try:
...
aborting... :("
        ...
        try:
            print(":) loading staging settings, sit tight")
            from config.settings.staging import *
        except ModuleNotFoundError:
            print(
                "No staging settings found, if you're in \
