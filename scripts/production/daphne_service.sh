...
echo "$SERVICE_CONTENT" | sudo tee /etc/systemd/system/booking-api.service > /dev/null
echo "$WORKER_CONTENT" | sudo tee /etc/systemd/system/booking-api-worker.service > /dev/null

# Reload systemd to pick up the changes
sudo systemctl daemon-reload
sudo systemctl start booking-api
sudo systemctl enable booking-api
sudo systemctl restart booking-api

sudo systemctl daemon-reload
sudo systemctl start booking-api-worker
sudo systemctl enable booking-api-worker
sudo systemctl restart booking-api-worker
