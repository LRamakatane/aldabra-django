...
import subprocess
import pathlib
from exceptions import NoOptionPassed, OptionNo
...
def create_db(db_name: str, user: str, h=None, p=None, E=None):
    ...
    if type(db_name) and type(user) != str:
        TypeError(
            "unexpected argument type provided for db_name or user\
            , must be a string"
        )
    ...
    create_db_args = (
        f"createdb/-U {user}/-h{host}/-p {port}/-E {enconding}/{db_name}".split(sep="/")
    )
